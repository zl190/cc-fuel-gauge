...
# When zone is red, handoff is enabled, and hasn't fired this session, trigger background handoff
HANDOFF_TRIGGER="${SCRIPT_DIR}/lib/handoff-trigger.sh"
HANDOFF_LOCK="/tmp/cc-fuel-gauge-handoff-${SESSION_ID}.lock"
if [ "$ZONE" = "red" ] && [ "$CFG_HANDOFF_ENABLED" = "true" ] && [ -f "$HANDOFF_TRIGGER" ] && [ ! -f "$HANDOFF_LOCK" ]; then
